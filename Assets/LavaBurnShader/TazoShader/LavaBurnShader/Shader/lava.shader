...
Shader "Tazo/lava"
{
	Properties
	{
		...
		_controll_x("Control X", Range(0, 20)) = 0
...
//shell
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				fixed4 qq =saturate( lerp(lerp(lerp(mt, px, _controll_x*(i.xyz.x-0.5)+0.5),py, _controll_y*(i.xyz.y-0.5)+0.5),pz, _controll_z*(i.xyz.z-0.5)+0.5));
				//fixed4 qq = saturate(lerp(lerp(px, py, _controll_y*(i.xyz.y - 0.5) + 0.5), pz, _controll_z*(i.xyz.z - 0.5) + 0.5));
