...
namespace BNG
{
    ...
    public class GrabbablesInTrigger : MonoBehaviour
    {
        ...
        public virtual Grabbable GetClosestGrabbable(Dictionary<Collider, Grabbable> grabbables,
                                                     bool remoteOnly = false, bool raycastCheck = false)
        {
            _closest = null;
            _lastDistance = 9999f;
        ...
        {
            RaycastHit hit;
            if (Physics.Linecast(startingPosition, theGrabbable.transform.position, out hit, RemoteCollisionLayers,
        ...
        protected virtual bool isValidGrabbable(Collider col, Grabbable grab)
        {
            ...
            {
                if (grab.BreakDistance > 0 &&
                    Vector3.Distance(grab.transform.position, transform.position) > grab.BreakDistance)
                {
                    return false;
                }
            }
        ...
        public virtual Dictionary<Collider, Grabbable> SanitizeGrabbables(Dictionary<Collider, Grabbable> grabs)
        {
            ...
            foreach (var g in grabs)
            {
                if (g.Key != null && g.Key.enabled && g.Value.isActiveAndEnabled)
                {
                    ...
                    if (g.Value.BreakDistance > 0 && Vector3.Distance(g.Key.transform.position, transform.position) >
                        g.Value.BreakDistance)
                    {
                        continue;
                    }
        ...
        public virtual void AddValidRemoteGrabbable(Collider col, Grabbable grabObject)
        {
            ...
            try
            {
                if (grabObject != null && grabObject.RemoteGrabbable && col != null &&
                    !ValidRemoteGrabbables.ContainsKey(col))
                {
        ...
        {
            ValidRemoteGrabbables.RemoveNullKeys();
            NearbyGrabbables.RemoveNullKeys();
            ValidGrabbables.RemoveNullKeys();
        }
